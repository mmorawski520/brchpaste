...
var currentPage=0;
 ...
 function get_logged(){
 	 ...
 	 $.ajax({
            type:'POST',
            url:'ifPassword.php',
            data:{'hash':urlParams.get("hash"),'isPassword':isPassword},
            success:function(html){
            	console.log("lel"+html)
            	if(html=="no logged"){
            		$("#passwordError").html("błędne hasło")
            		console.log("xdd")
            	}
            	if(html!="no logged" && html!="" && html!="doesnt exits" && isPassword!=""){
            		$(".main").empty()
 ...
 if(urlParams.get('hash')==null){
  ...
  $.ajax({
	...
	isPassword=true;
	$(".main").append("<label>Podaj hasło<br><span style='color:red;font-size:.5em;'id='passwordError'></span><br><input id='getPassword' name='GetPassword' type='password'></label><br><input onclick='get_logged()' type='submit' value='zaloguj'>")
}
            }
          })
 }
